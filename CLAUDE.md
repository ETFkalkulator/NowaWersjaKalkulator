...
css/
  tailwind.css                    — jedyny plik CSS ładowany w HTML (skompilowany build)
  tailwind-input.css              — źródło do kompilacji (nie deployować)
  reset.css / main.css / variables.css / mailerlite.css / print.css  — źródła (nie deployować)
...
3. **Jedyny plik CSS w HTML:** Każda strona ładuje wyłącznie `css/tailwind.css` (lub `../css/tailwind.css` z podstron). Pliki `reset.css`, `variables.css`, `main.css` są źródłami do buildu — **nie trafiają na serwer**.
...
## Synchronizacja z vault-brain

Osobista baza wiedzy projektu: `C:/Users/komptest/Documents/vault-brain/`

### Kiedy sięgać po wiki (selektywnie)

Przed podjęciem decyzji lub odpowiedzią na pytanie z tych obszarów — przeczytaj `index.md` i relevantne strony wiki:

- **SEO / ruch / CTR** — frazy, pozycje, priorytety (np. GSC dane)
- **Treść / artykuły** — co jest już opisane, jak opisano konkretne produkty (ETF, IKE, obligacje)
- **Dylematy architektoniczne** — co było omawiane wcześniej, jakie decyzje podjęto
- **Dane finansowe** — stawki obligacji, limity IKE/IKZE, przykłady liczbowe (wiki jest źródłem prawdy)
- **Wywołanie:** użytkownik napisze `Query:` lub `sprawdź bazę`

Nie sprawdzaj wiki przy: bugfixach CSS/JS, prostych edycjach HTML, pytaniach czysto technicznych.

**Jak sprawdzić:**
1. Przeczytaj `C:/Users/komptest/Documents/vault-brain/index.md`
2. Przeczytaj strony relevantne do pytania
3. Opieraj odpowiedź/decyzję na znalezionej wiedzy

### Komenda: `Sync to brain`

Po sesji roboczej zapisz podsumowanie do vault-brain:

1. Utwórz plik `C:/Users/komptest/Documents/vault-brain/raw/projekty/etf-kalkulator/YYYY-MM-DD-session.md`
2. Użyj formatu:
...
type: session-log
project: etf-kalkulator
created: YYYY-MM-DD
tags: [etf-kalkulator, session-log]
---
...
- [decyzje podjęte i uzasadnienie — "dlaczego tak?"]

## Kontekst do następnej sesji
- [co warto wiedzieć zaczynając następną sesję]

## Otwarte kwestie
- [nierozwiązane problemy lub pytania]
```

3. Poinformuj użytkownika: "Plik sesji zapisany. W vault-brain napisz `Ingest all` aby przetworzyć go do wiki."
