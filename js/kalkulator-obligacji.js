...
const BOND_CONSTANTS = {
    ...
    ROR: { name: 'ROR (1 roczne)', years: 1, fixedYr1: 0.0375, margin: 0.0000, penalty: 0.50, cap: 'payout', tax: 'payout' },
    ROS: { name: 'ROS (6 letnie)', years: 6, yr1: 0.0580, margin: 0.0200, penalty: 1.50, cap: 'annual', tax: 'end' },
    ROD: { name: 'ROD (12 letnie)', years: 12, yr1: 0.0600, margin: 0.0250, penalty: 2.00, cap: 'annual', tax: 'end' }
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    ['input-typ', 'input-ike', 'input-800plus', 'input-rolowanie', 'input-wykup'].forEach(id => {
        ...
        if (el) el.addEventListener('change', () => {
            if (id === 'input-800plus') updateTypeOptions();
            if (id === 'input-typ') updateEarlyRedemptionSlider();
            obliczWszystko();
        });
    });
...
function updateTypeOptions() {
    ...
    Array.from(typeSelect.options).forEach(opt => {
        if (opt.value === 'ROS' || opt.value === 'ROD') {
            opt.disabled = !is800;
            if (!is800) opt.classList.add('hidden');
            else opt.classList.remove('hidden');
        }
    });

    if (!is800 && (currentVal === 'ROS' || currentVal === 'ROD')) {
...
function initChart() {
    ...
    myChart = new Chart(ctx, {
        ...
        options: {
            ...
            plugins: { legend: { display: false }, tooltip: { cornerRadius: 12, padding: 16 } },
            scales: {
                x: { grid: { display: false }, ticks: { color: textColor } },
                y: { grid: { borderDash: [5, 5] }, ticks: { color: textColor, callback: (v) => window.formatujZl ? window.formatujZl(v).split(',')[0] + ' zł' : v } }
            }
        }
    });
}
